...
final class AuthenticateByIp
{
    ...
    public function __invoke(string $ip): Result
    {
        $user = $this->authRepository->findByHost($ip);
        if ($user === null) {
            return Result::failure(new \RuntimeException('No single user for this IP'));
        }
